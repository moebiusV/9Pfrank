...
Recommended initial deployment: persistent TLS 1.3 over TCP, a userspace client and server, bounded pipelining, compound metadata operations, and conservative caching. Add reconnect recovery, coherent caching, and QUIC only after correctness and measurements justify them. WireGuard is an excellent deployment option where an encrypted host network already exists.
...
Feature IDs: CORE=1, POSIX=2, XATTR=3, LOCK=4, COMPOUND=5, RECOVERY=6, WATCH=7, LEASE=8, SPACE=9, COPY=10, AUTHFILE=11, PLAN9_META=12, ACL_POSIX=13. CORE revision 1 is required; the full filesystem profile requires 1..7, 9..13 where the backend supports their semantics, and explicitly reports backend limitations. LEASE is optional; correct uncached operation remains possible. QUIC is a separately versioned transport profile, not a filesystem capability bit.
...
| Mobile/lossy WAN with many independent operations | Experimental QUIC transport | Independent stream delivery can reduce transport blocking; measure CPU, UDP reachability, and library behavior |
...
QUIC provides ordered streams without imposing ordering between different streams, plus transport flow control. It still shares congestion constraints, and a lost packet can affect frames from several streams. Use one control stream for bootstrap/HELLO and, after negotiation, one bidirectional stream per ordinary request/reply. Each carries one complete request and one complete reply. Control requests stay on the control stream; TLS protection is intrinsic to QUIC. [QUIC transport, RFC 9000](https://www.rfc-editor.org/rfc/rfc9000), [QUIC TLS mapping, RFC 9001](https://www.rfc-editor.org/rfc/rfc9001)

Before enabling QUIC, specify cross-stream dependencies: clients must await prior operations on a fid before starting another stream using it, or use a compound. Different fids can proceed concurrently. Closing a QUIC stream is not proof that an operation did not execute; CANCEL and replay semantics still apply. Keep TCP as a configured secure fallback, never downgrade to plaintext. `9p2026` is a proposed ALPN identifier; check registration requirements before publication. Use configurable ports until service registration is settled.
...
Compare isolated-lab plaintext TCP as a measurement baseline, TLS/TCP, WireGuard/TCP, and later QUIC. Never make the baseline an exposed production mode.
...
### Phase F — optional coherency and WAN transport

Specify and model read leases before implementation. Verify interactions with local backend writers, notification overflow, suspend, time drift, session expiry, and authorization changes. Prototype QUIC with explicit same-fid dependencies and compare it to TCP under realistic loss.

Gate: either demonstrate the cache/transport benefit with correctness evidence, or keep the module optional and disabled. A useful 2026 release does not depend on writable leases, distributed transactions, or transparent failover.
