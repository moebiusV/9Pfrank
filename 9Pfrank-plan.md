...
Use one new dialect, `9Pfrank`, with explicit capability negotiation. The core implements one unified operation set: the semantic union of every dialect's operations. Everything that is not 9Pfrank is a codec that translates its wire protocol to 9Pfrank calls underneath, which is straightforward because 9Pfrank is a superset; each codec is therefore minimal, a shim rather than a reimplementation. Wire formats stay per-dialect and are not reinterpreted; what is unified is the semantics, not the bytes. Where a dialect differs structurally, the codec performs a small, documented translation rather than requiring the native set to mirror it; for example, 9P2000 `Tcreate` turns the directory fid into the new file's fid, and legacy partial walks remain a codec behavior. The rules in this document bind 9Pfrank proper; each legacy codec speaks its dialect as that dialect defines it, including its own transport and authentication.
...
1. The server has four listeners. The TLS port expects a TLS ClientHello first and, after the handshake, runs step 2 detection, which selects `9Pfrank` or a TLS-wrapped legacy codec. The plain TCP port carries raw bytes, where step 2's byte-0 dispatch selects any dialect, native `9Pfrank` included. The 9front port runs dp9ik pre-auth then TLS-PSK and accepts only the 9P2000 codec. The PAKE port runs a standalone PAKE (OPAQUE or SPAKE2+) in plaintext, then keys TLS-PSK from its output for native 9Pfrank. The operator enables whichever listeners they want; tunnel binding is reported, not gated.
...
Native 9Pfrank gets password-only authentication without certificates the same way, on its own port: a standalone PAKE (OPAQUE or SPAKE2+) runs in plaintext, and its output keys TLS 1.3 through the external-PSK importer (RFC 9258) in `psk_dhe_ke` mode, so forward secrecy holds and a later password compromise cannot decrypt recorded sessions. The session ends encrypted, with no CA and no pinning.
