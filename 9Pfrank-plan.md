...
Build a semantic superset of 9P2000, 9P2000.u, 9P2000.L, 9P2000.e, and 9P.original. 9Pfrank copies 9P2000, then 9P2000.u, then 9P2000.e, and folds in the remaining 9P2000.L material as the happy path. Preserve 9P's central model: attach to a namespace, obtain fids, walk names, read and write resources, and release references. Support synthetic files and services as first-class resources, not merely disk files. 9P.original (editions 1 through 3) is a distinct pre-version wire format, supported through its own codec like the other legacy dialects. The design is also inspired by filsys, a library for the family of pre-FFS Unix filesystems; 9Pfrank is filsys for 9P. The filsys decisions that carry over, where practical: one parser per format (one codec per dialect); invariant-based detection, a violation meaning it is not that format; fieldwise decoding with no host-struct casting; explicit overflow rather than silent truncation; self-delimiting parsers; and a FUSE adapter that mounts the format as a living namespace.
...
Native 9Pfrank gets password-only authentication without certificates on its own port. The PAKE is SPAKE2+ (RFC 9383), with OPAQUE only by negotiation. The preamble is client-first and framed: a method byte, a user identifier, and the SPAKE2+ exchange, with unknown and wrong-password users answered identically so the server does not enumerate users. The PAKE context binds the server identity, the service name or port, and the ALPN id `9Pfrank`, so a run cannot be relayed to a different service. The output keys TLS 1.3 through the external-PSK importer (RFC 9258) in `psk_dhe_ke` mode, so forward secrecy holds and a later password compromise cannot decrypt recorded sessions. After TLS the port carries native 9Pfrank only, and `authfid=0` means the PAKE-authenticated user. The session ends encrypted, with no CA and no pinning.
