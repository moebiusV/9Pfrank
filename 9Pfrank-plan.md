...
- **One binary, every dialect.** filsys ships one binary that mounts every edition. 9Pfrank ships `lib9pfrank`, which holds both the server and the client, speaking every dialect through the codecs. The two roles share one API, as the BSD sockets API does not split connect from listen and accept.
