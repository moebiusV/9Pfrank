...
- **Tools beyond the mount.** filsys ships `mkfs`, `fsck`, and `findfs` alongside the read/write mount: create, check, and locate a filesystem, not merely read one. The 9Pfrank analogs: a probe tool that connects to a server and reports its dialects, listeners, and auth methods (`findfs`); a conformance checker that runs the Phase B and G suites against any server (`fsck`); and a wire tracer/dumper in the tradition of Plan 9's `aux/9pcon`.
