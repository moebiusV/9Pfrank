...
- **Tools beyond the mount.** filsys ships `mkfs`, `fsck`, and `findfs` alongside the read/write mount: create, check, and locate a filesystem, not merely read one. `findfs` is the analog of 9Pfrank's own autodetect (§3 step 2); there is no `fsck` analog. A wire tracer/dumper follows Plan 9's `aux/9pcon`.
