...
12. **The Time Capsule**: the grand finale, a server whose files live in any pre-FFS Unix image (V0 through V7, 32V, Coherent, Xenix, 2.9/2.11BSD). The server links `libfilsys` directly as a backend and exports the image natively, with no FUSE in the server path.
...
- **One engine, codecs on top.** A single descriptor plus a per-format ops vtable covers the whole range; only what genuinely differs on disk survives as a vtable op. 9Pfrank keeps one native engine, and each legacy dialect is a codec exposing only what differs: header, opcode map, stat, transport.
...
- **Tools beyond the mount.** filsys ships `mkfs`, `fsck`, and `findfs` alongside the read/write mount: create, check, and locate a filesystem, not merely read one.
...
- **Invariant-based detection.** filsys probes every format in two passes, strong first, and treats a violated invariant as not-that-format. 9Pfrank detects 9P.original (byte-0 `Tnop`/`Tsession`) and the version path the same way: invariants only. The spec reports no confidence tier.
