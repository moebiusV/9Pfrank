...
Build a semantic superset of 9P2000, 9P2000.u, 9P2000.L, 9P2000.e, and 9P.original. 9Pfrank assembles its operation set by copying 9P2000 first, then 9P2000.u, then 9P2000.e, then folding in the remaining 9P2000.L material; `.L`'s POSIX semantics are the default behavior (the happy path). The copying order is about the operation set, not legacy wire forms. Preserve 9P's central model: attach to a namespace, obtain fids, walk names, read and write resources, and release references. Support synthetic files and services as first-class resources, not merely disk files. 9P.original (editions 1 through 3) is a distinct pre-version wire format, supported through its own codec like the other legacy dialects. The design is also inspired by filsys, a library for the family of pre-FFS Unix filesystems; 9Pfrank is filsys for 9P. The filsys decisions that carry over are enumerated in §13.

Use one new dialect, `9Pfrank`, with explicit capability negotiation. The core implements one unified operation set: the semantic union of every dialect's operations. Everything that is not 9Pfrank is a codec that translates its wire protocol to 9Pfrank calls underneath, which is straightforward because 9Pfrank is a superset; each codec is therefore minimal, a shim rather than a reimplementation. Each codec runs in both directions: on the server it decodes legacy wire messages into 9Pfrank calls; on the client it encodes 9Pfrank calls as legacy requests and decodes legacy replies. The client direction has its own translation work: a few native operations lack a direct legacy form and are emulated or refused per dialect, as Phase A's coverage inventory enumerates. Most codec work is stateless translation; a few pieces keep per-session state or real logic: the collision-free qid mapping, synthesizing stable `ObjectRef` IDs from qids plus server identity, `..` and partial-walk emulation, `Tcreate`'s fid replacement, the sequence-to-tag map, the 9P.original self-delimiting parser, and `.e` session restore. Wire formats stay per-dialect and are not reinterpreted; what is unified is the semantics, not the bytes. Where a dialect differs structurally, the codec performs a small, documented translation rather than requiring the native set to mirror it; for example, 9P2000 `Tcreate` turns the directory fid into the new file's fid, and legacy partial walks remain a codec behavior. The rules in this document bind 9Pfrank proper; each legacy codec speaks its dialect as that dialect defines it, including its own transport and authentication.

Porting a 9P client to 9Pfrank means swapping its codec (20-byte header, `Result` prefixes, `ObjectRef` for qids, sparse `Attributes` for `stat`) and handling a short set of semantic differences (HELLO before attach, all-or-nothing walk, CREATE keeping the directory fid, GETATTR/SETATTR for stat/wstat). The operation model is unchanged. 9P.original clients cannot switch this way.
...
1. The server has five listeners. The TLS port expects a TLS ClientHello first and, after the handshake, runs step 2 detection, which selects `9Pfrank` or a TLS-wrapped legacy codec. The plain TCP port carries raw bytes, where step 2's byte-0 dispatch selects any dialect, native `9Pfrank` included. The Unix-socket port carries raw bytes with the same step 2 detection, and its peer credentials are the transport identity for `authfid=0`. The 9front port runs dp9ik pre-auth then TLS-PSK and accepts only the 9P2000 codec. The PAKE port runs SPAKE2+ in plaintext, then keys TLS-PSK from its output for native 9Pfrank. The operator enables whichever listeners they want; tunnel binding is reported, not gated.
...
5. By default, a 9Pfrank server accepts 9P2000, 9P2000.u, 9P2000.L, 9P2000.e, and 9P.original clients. Local policy may restrict the permitted set; a dialect outside it is disconnected. A compatibility retry uses a fresh connection and the exact selected legacy codec.
...
Authentication establishes an allowed principal set. A UID/GID in a packet never proves identity; uname trust is an explicit server policy that accepts a stated name without making a numeric UID or GID authoritative, and UID zero never implies remote root. Fids inherit the authorized principal and export of their attach; operations cannot swap credentials. Group memberships are server-validated. A trusted multiuser gateway requires explicit impersonation authority; a machine certificate alone is insufficient.
...
- **Auth files:** AUTH creates a negotiated-method fid; READ/WRITE exchange method-defined bounded records. ATTACH uses authfid=0 for transport identity, or a completed auth fid otherwise. Over plain TCP there is no transport identity, so authfid=0 falls back to the stated identity in `requested`, subject to server policy; that is the classic 9P2000 uname trust, not a real method. A real method without TLS is a PAKE: a standalone PAKE (SPAKE2+ or OPAQUE) avoids the 9front auth-server dependency that dp9ik needs. Authentication does not protect the session itself; over plain TCP it remains injectable after the exchange, and a PAKE-derived key can key TLS-PSK, giving password-only auth that ends encrypted with no CA and no pinning. AUTHFILE methods must bind their result to the session/export. Sending credentials over an unencrypted connection is a warned config choice, not a prohibition.
...
Security is made easy and transparent, not forced. Nothing that worked before stops working, and the secure path wins because it is easier, not because the other path is locked. The operator can see what is and is not encrypted, so plaintext is always visible.
...
| Plain TCP | `9Pfrank` or any legacy dialect, as before | No confidentiality or integrity; a session can be hijacked after authentication |
...
TLS is made the low-friction default: the server generates its identity at install, clients pin the key on first use or enroll with one command, and `tls://host` is as short to type as `tcp://host`.
...
Native 9Pfrank gets password-only authentication without certificates on its own port. The PAKE is SPAKE2+ (RFC 9383), with OPAQUE only by negotiation. SPAKE2+ sends per-user parameters to the client (the salt and the password-hashing settings) before the exchange, so the server must answer unknown and wrong-password users identically to avoid enumerating accounts: it returns deterministic fake parameters for unknown users, derived from a server secret and the username, as OPAQUE specifies fake records. A registration step stores the verifier for a user; it runs in the one-command enrollment. The preamble is client-first and framed: a method byte, a user identifier, and the SPAKE2+ exchange. The PAKE context binds the server identity, the service name or port, and the ALPN id `9Pfrank`, so a run cannot be relayed to a different service. The output keys TLS 1.3 through the external-PSK importer (RFC 9258) in `psk_dhe_ke` mode, so forward secrecy holds and a later password compromise cannot decrypt recorded sessions. After TLS the port carries native 9Pfrank only, and `authfid=0` means the PAKE-authenticated user. The session ends encrypted, with no CA and no pinning.
...
A `tls://` mount does not silently fall back to plaintext; a `tcp://` mount stays unrestricted. `9Pfrank` is a proposed ALPN identifier; check registration requirements before publication. Use configurable ports until service registration is settled.

### 9.2 Mount security policy

- Authenticate the server using a configured CA/name or pinned public key. Interactive first contact may pin the key on first use, showing the fingerprint; unattended mounts take their pin from the one-command enrollment, done interactively once, and never automate trust-on-first-use. Rotate keys with overlap and revocation procedures. Keep private keys out of world-readable mount configuration.
...
Compare isolated-lab plaintext TCP as a measurement baseline, TLS/TCP, and WireGuard/TCP.
...
EXEC alone (4) is the only refusal 9P.original shares with the rest; its `CHAPPEND`/`CHEXCL` create flags are to confirm in Phase A. The emulations preserve client-visible semantics: compounds promise ordering, not atomicity, and the clone reproduces the native outcome. CANCEL's `Tflush` mapping is coarser: a reply that arrives before `Rflush` maps to COMPLETED (flush(5) honors it as if it had not been flushed), and an `Rflush` with no reply maps to CANCELLED with effect UNKNOWN, because 9P2000 does not guarantee a flushed request left no trace. Add tests for old auth/stat encodings, numeric identity preference, partial walks, remove/clunk semantics, and the canonical 9P2000 error strings (Plan 9 programs match strings, e.g. NOT_FOUND to "file does not exist"). Review Plan 9 synthetic resource workloads, not only POSIX files.
...
Gate: two independent codecs agree byte-for-byte on little- and big-endian, 32- and 64-bit hosts (the qemu CI runs s390x and m68k for big-endian, armhf for 32-bit), malformed inputs remain within bounded memory/CPU, and every draft field has defined validity and failure behavior.
...
11. **FUSE Read-only Mount**: the 9Pfrank client's FUSE driver mounts the remote namespace read-only. Interrupt-to-CANCEL mapping and lock owners stay in the full driver. macOS goes through macFUSE; FSKit is not FUSE-compatible.
12. **The Time Capsule**: the grand finale, a server whose files live in any pre-FFS Unix image filsys reads. The server links `libfilsys` directly as a backend and exports the image natively, with no FUSE in the server path.

These twelve projects are the `examples/` directory; each ships in C against the library and in idiomatic newLISP through the newLISP FFI binding. Because newLISP is single-threaded, the library exposes one caller-owned, poll-driven (non-blocking, no internal threads) event loop shared by the client and the server, so the binding delivers callbacks on the interpreter thread and a newLISP program can act as client and server at once, or as a proxy. The cookbook links filsys; record its license alongside 9Pfrank's ISC.
...
## 13. Design lineage: filsys

filsys is a library for the family of pre-FFS Unix filesystems (PDP-7 through V10, plus 32V, Coherent, Xenix, 2.9/2.11BSD, and System III/V); 9Pfrank is filsys for 9P. The decisions that carry over, where practical:

- **One engine, codecs on top.** A single descriptor plus a per-format ops vtable covers the whole range; only what genuinely differs on disk survives as a vtable op. 9Pfrank keeps one native engine, and each legacy dialect is a codec exposing only what differs: header, opcode map, stat/attribute translation, and the stateful pieces named in §1. Transport belongs to the listeners (§3 step 1), not the codec.
- **One binary, every dialect.** filsys ships one binary that mounts every edition. 9Pfrank ships `lib9pfrank`, which holds both the server and the client, speaking every dialect through the codecs. The two roles share one library and one handle type, with connect for clients and listen/accept for servers, as in BSD sockets.
- **The distro matrix.** filsys builds and runs on Linux (six distros: Alpine/musl, Debian, Ubuntu, Fedora, openSUSE, Arch), FreeBSD, NetBSD, OpenBSD, illumos, DragonFly BSD, HardenedBSD, MidnightBSD, GhostBSD, and macOS, with Windows through WSL2. One FUSE adapter per platform is chosen by configure, never by #ifdef. §12 covers backend semantics, not this set. filsys reaches Windows through WSL2, a Linux backend; a native Windows backend would be its own §12 row, not yet enumerated.
- **The architecture matrix.** filsys is exercised on 64- and 32-bit hosts in both byte orders: amd64 (64 LE), s390x (64 BE), m68k (32 BE), and armhf (32 LE), cross-compiled with gcc and run under qemu. Codecs read and write through an explicit byte-order vtable, never a host-struct cast. 9Pfrank's fieldwise codecs and little-endian native header follow the same rule.
- **The C rules.** filsys minimizes the preprocessor to near zero: `#define` for header guards, everything else `const` or `enum`. 9Pfrank keeps C99 with the same no-macro rule, enum constants for array sizes and case labels, and `static const` for u64 masks.
- **GNU packaging.** `./configure && make && make install` with a shipped `configure` (no autotools needed), the standard targets, and a manpage per tool. 9Pfrank's deliverables carry the same GNU autotools layout.
- **Tools beyond the mount.** filsys ships `mkfs`, `fsck`, and `findfs` alongside the read/write mount: create, check, and locate a filesystem, not merely read one. `findfs` is the analog of 9Pfrank's own autodetect (§3 step 2); there is no `fsck` analog. Debugging tools follow the Plan 9 precedents: a `9pcon`-style console for typing raw messages and reading replies, and a `chatty9p`-style message tracer.
- **A linkable core, with FUSE as one adapter.** filsys ships `libfilsys` (the linkable, FUSE-free core) and a thin FUSE mount over it. 9Pfrank ships `lib9pfrank` the same way. filsys is a 9Pfrank server backend: the server links `libfilsys` directly and exports the image as a native namespace, with no FUSE in the server path. FUSE appears on the client, which mounts the remote namespace locally.
- **Invariant-based detection.** filsys probes every format in two passes, strong first, and treats a violated invariant as not-that-format. 9Pfrank keeps the invariant rule but reverses the order: it checks byte-0 `Tnop`/`Tsession` first, because an original-9P client sends a 3-byte `Tnop` and waits, so the server cannot hold out for the 13 bytes a strong `Tversion` check needs. The spec reports no confidence tier.
- **Equivalence classes, conditional on Phase A.** filsys reports byte-identical editions as one class ("v1, v2 or v3") with one canonical token. Whether 9P.original editions 1 through 3 collapse onto one wire form is not yet established; the authentication messages may differ, so the class claim for 9P.original is conditional on the Phase A framing (decision #8).
- **Exact dialect names are never reinterpreted.** filsys treats a named edition that fails to match as a hard error. 9Pfrank carries this over only to exact dialect names: a client that names a dialect and mismatches is disconnected. Unrecognized `9P…` strings still get the standard version(5) negotiation (reply `9P2000`), which the client may decline.
- **Explicit overflow and self-delimiting parsers.** An on-disk maxima table bounds each edition, every size is checked against the medium before any allocation or loop, and each codec carries a one-line invariant for how much it reads. 9Pfrank checks frame size and counts against limits and reports OVERFLOW rather than truncating; its header carries the body length, and the 9P.original codec delimits its own messages over any stream.
...
- **Ordered writes and single-writer access are backend properties.** filsys enforces soft-updates write ordering and an advisory single-writer lock. 9Pfrank makes no protocol-level single-writer claim: the full-filesystem profile is POSIX multi-writer, and exclusivity is `DMEXCL`/EXCLUSIVE_OPEN plus locks and leases. Backends that need ordered writes or single-writer access, such as the filsys backend in cookbook #12, enforce them and report conflicts explicitly.
